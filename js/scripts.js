$(document).ready(funciton(){
  $("form#quiz").submit(function(event){
    ...
    if (!quantity || !mehod || !consistency || !omelette || !bird){
      (alert("Please enter a response for each question before submitting");
    }
    else {
      ("#result").text();}
      if (quantity === 1 || method === overmedium || method === scrambled || consistency === hard || (omelette === denver && bird === chicken) || bird === pheasant){
        $("option1").show();
      }
